...
export default function FindTheDay() {
  ...
  useEffect(() => { generateDate(); }, []);
  useEffect(() => { if (date) calculateAnswers(date); }, [date]);
    ...
    <div>
        ...
        {!showSolution ? (
          <>
            <div>
              ...
              <select value={anchorInput} onChange={(e) => setAnchorInput(e.target.value)}
                ...
                {DAY_NAMES.map((name, i) => (
                  <option key={i} value={i} className="bg-gray-900">{name}</option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <select value={doomsdayInput} onChange={(e) => setDoomsdayInput(e.target.value)}
                className={`w-full rounded-lg px-4 py-2.5 text-sm border ${stateClasses[doomsdayState]}`}>
                <option value="" disabled>Select doomsday…</option>
                {DAY_NAMES.map((name, i) => (
                  <option key={i} value={i} className="bg-gray-900">{name}</option>
                ))}
              </select>
            ...
            <div>
              ...
              <select value={finalDayInput} onChange={(e) => setFinalDayInput(e.target.value)}
      ...
      {(hasGuessed || showSolution) && (
        <div className="bg-white/[0.03] border border-white/10 rounded-xl px-8 py-5 mb-4 flex flex-col gap-2 text-sm font-mono">
          <FeedbackRow label="Anchor" state={anchorState} correct={correctAnchor} />
          <FeedbackRow label="Doomsday" state={doomsdayState} correct={correctDoomsday} />
          <FeedbackRow label="Final Day" state={finalDayState} correct={correctFinalDay} />
        </div>
      )}
...
function FeedbackRow({
  ...
  return (
    <div className="flex justify-between">
      ...
      <span className={state === "correct" ? "text-emerald-400" : "text-red-400"}>
        {state === "correct"
          ? "Correct"
          : correct !== null
